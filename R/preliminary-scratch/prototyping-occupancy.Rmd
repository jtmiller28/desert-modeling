...
touching_cells <- grid_2[sonoran_shp_b,]
...
range <- st_convex_hull(st_union(filter(occ_f, accepted_name == "Rafinesquia neomexicana")))

## Graphically represent this 
grid1_plot <- ggplot()+
  geom_sf(sonoran_shp, mapping = aes()) +
  geom_sf(touching_cells, mapping=aes(), fill=NA, color="grey")+
  geom_sf(range, mapping=aes(), fill="cyan", color="cyan", alpha=0.35) +
  ggtitle("Rafinesquia neomexicana's occurrence range over the Sonoran Desert")
  theme_map()
